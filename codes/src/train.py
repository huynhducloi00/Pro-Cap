...
def eval_multi_model(epoch, opt, model):
    ...
    for k in trange(1):
        test_loader = DataLoader(
            Multimodal_Data(opt, opt.DATASET, "test"),
            opt.BATCH_SIZE,
            shuffle=False,
            num_workers=2,
        )
    ...
    pickle.dump(
        [
            list(labels_record.keys()),
            list(input_record.values()),
            probs.cpu(),
            labels.cpu(),
        ],
        open(f"loi_{epoch}.txt", "wb"),
    )
    # print (auc)
    return scores * 100.0 / len_data, auc * 100.0 / len_data
...
def train_for_epoch(opt, model, train_loader, test_loader):
    ...
    for epoch in trange(opt.EPOCHS):
        ...
        if opt.MODEL == "pbm" and opt.USE_DEMO and opt.MULTI_QUERY:
            eval_acc, eval_auc = eval_multi_model(epoch, opt, model)
        else:
